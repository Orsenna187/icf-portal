<script>
	...
	$effect(() => {
		if ($authStore.loading || publicRoutes.includes($page.url.pathname)) {
			return;
		}
...
{:else}
	<div class="min-h-screen flex flex-col bg-base-100">
		<nav class="navbar border-b border-base-200 shadow-md">
			...
			<div class="hidden md:flex gap-4 px-4 items-center">
				{#if $authStore.user}
					...
					<button class="btn btn-ghost btn-square" title="Logout" onclick={handleLogout}>
						<MdiLogout class="w-6 h-6" />
					</button>
				{/if}
			</div>
			...
			<div class="md:hidden border-b border-base-200 shadow-md" transition:slide>
				<div class="flex flex-col p-4 gap-2">
					{#if $authStore.user}
						...
						<button class="btn btn-ghost justify-start gap-4" onclick={handleLogout}>
							<MdiLogout class="w-6 h-6" />
							<span>Logout</span>
						</button>
					{/if}
		...
		<div class="flex flex-1 overflow-hidden">
			{#if $authStore.user}
				<nav class="hidden md:flex flex-col w-72 border-r border-base-200 shadow-lg p-4 overflow-y-auto">
			...
			<main class="flex-1 bg-gradient-to-br from-base-100 via-base-200/50 to-base-300/25 overflow-y-auto">
				{@render children()}
			</main>
		</div>
	</div>
